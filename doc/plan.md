...
根据你的实际经验（处理Kühne数据 + 发现Pandas慢 + 想用并行计算），我来帮你**一步步规划这个作业**。
...
| 你的经验                          | 对应作业要求            |
| --------------------------------- | ----------------------- |
| 处理过Kühne的Sell-out/Sell-in数据 | ✅ 有真实数据场景       |
| 发现Pandas读Excel慢               | ✅ 可以做为顺序基线     |
| 有多个文件（按周/月分区）         | ✅ 适合并行处理         |
| 需要做映射/聚合                   | ✅ 计算密集型，适合并行 |
| 数据量大（GB级别）                | ✅ 需要分布式           |
...
```python
"""
场景：Kühne公司需要分析2024年EDEKA门店的销售数据
数据：52个Excel文件，每个约200MB，总计~10GB
...
> **你的期末作业 = 把你现在做的Kühne数据处理，用顺序(慢)、并行(快)、分布式(更快)三种方式实现，然后对比证明——这就是教授要的！**
